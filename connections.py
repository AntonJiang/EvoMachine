...
class Connections():
    ...
    def update(self, meta_variant, neuron, neurons, distance):
        ...
        birth_neurons = rng.choice(neurons, size=e_utils._deter_size(rng, Hyperparam.connection_life_percent/2, \
                                                        Hyperparam.connection_life_percent_variant/2, self.size), \
            replace=False)
    ...
    def _update(self, index, meta_variant):
        ...
        self.states = [_flip(state) for state in self.states if (rng.random() < Hyperparam.connection_state_flip_percent)]
        function_index = rng.choice(np.arange(self.size), size=e_utils._deter_size(rng, Hyperparam.connection_function_percent \
                                                        Hyperparam.connection_function_percent_variant, self.size), \
            replace=False)
